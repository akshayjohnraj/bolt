...
class RecordButton(Button):
    ...
    def __init__(self, ting, talk):
        self.ting = ting
        self.talk = talk
        self.self.listener = sr.Recognizer()
    ...
    def record(self):
        ...
        try:
            value = self.self.listener.recognize_google(voice)
            self.output = "You said \"{}\"".format(value)

            if 'bolt' or 'bold' or 'gold' or 'v' or 'bol' or 'board' or 'cold' in command:
                command = command.replace('bolt', '').replace('bold', '').replace('gold', '').replace('v', '').replace(
                    'bol', '').replace('board', '').replace('cold', '')
                self.output = "You said \"{}\"".format(value)
                self.output = command
                print(command)
                ...
                self.talk('my name is bolt tell you command after saying my name Sir')
            if 'play' in command:
                song = command.replace('play', '')
                value = self.listener.recognize_google(voice)
                ...
                self.talk('hello, how can i help you ??')
                self.output = "You said \"{}\"".format(value)

            elif 'how are you' in command:
                print('Im fine sir how are you ')
                self.talk('Im fine sir how are you')
                self.output = "You said \"{}\"".format(value)

            elif 'translate ' in command:
                engine.setProperty('rate', 170)
                self.talk('ill ask my sister to translate wait sir')
                ...
                tts.save("audio.mp3")
                playsound("audio.mp3")
                self.output = "You said \"{}\"".format(
                    value) + 'ill ask my sister thrisha to translate wait sir' + out.text

            elif 'take note' in command:
                self.talk('enter the things you want to write as handwriting sir')
                ...
                self.talk('select the date and desired company to plot the graph')
                ...
                engine.setProperty('rate', 170)
                self.talk('ill ask my sister thrisha to translate wait sir')
                command = self.listener.recognize_google(voice, language="ta-IN")
                ...
                self.talk('On what number should I send the message sir? Please enter in the console: ')
                self.output = 'On what number should I send the message sir? Please enter in the console: '
                number = input("Enter the number: ")
                self.talk("What is the message sir?")
                self.output = 'What is the message sir?'
                message = takeCommand().lower()
                send_whatsapp_message(number, message)
                self.talk("I've sent the message sir.")
                ...
                if x["cod"] != "404":
                    ...
                    self.talk(" Temperature in kelvin unit is " +
                ...
                self.talk('tell the time to set alarm')
                ...
                self.talk('this is what i found in web sir')
                ...
                self.talk('Enter the ticker of company:')
                ...
                print('the answer is')
                self.talk('the answer is ')
                print(eval_binary_expr(*(command.split())))
                self.talk(eval_binary_expr(*(command.split())))
                ...
                self.talk('good bye, have a nice day !!')
                sys.exit()

            elif 'thank you' in command:
                print("your welcome")
                self.talk('your welcome')
                ...
                self.talk('Here is the location of ' + loc)
                ...
                self.talk('Here is the location of ' + loc[1])
                ...
                self.talk('sorry sir i dont know the answer')
                print(' Here is what i found on the internet..')
                self.talk('Here is what i found on the internet..')
                search = 'https://www.google.com/search?q=' + command
                webbrowser.open(search)


        except sr.UnknownValueError:
            self.output = self.talk("Oops! Didn't catch that")
            takeCommand()

        except sr.RequestError as e:
            self.output = self.talk("Uh oh! Couldn't request results from Google Speech Recognition service; {0}".format(e))
            takeCommand()
